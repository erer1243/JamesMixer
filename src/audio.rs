...
pub struct Audio {
    ...
    /// READ ONLY atomics related to current state of music being played
    music: SharedMusicState,
}
...
impl Audio {
    ...
    pub fn init() -> Audio {
        ...
        // Connect jack ports. This requires a second client, for some reason.
    ...
    /// Returns 00:00 for both values if no song is selected yet.
    ...
    pub fn pause_music(&self) {
        self.ac_send.send(AudioControl::Pause).unwrap();
    }

    /// Unpauses the current song. Does nothing if there is no song selected (is_music_paused()
    /// will still return true).
    pub fn unpause_music(&self) {
        self.ac_send.send(AudioControl::Unpause).unwrap();
    }
    ...
    /// If the timestamp is out of range, or no song was selected yet, it does nothing.
    ...
    /// Takes in a source and a volume value between 0-100
    pub fn set_volume(&self, source: Source, value: f32) {
        self.ac_send
            .send(AudioControl::SetVolume(source, value / 100.0))
            .unwrap();
    }
}

/// Struct that contains atomics that are shared between ui and audio thread
#[derive(Clone)]
...
fn audio_callback(out: &mut [f32], mic: &[f32], line: &[f32], music: &mut ThreadState) {
    use AudioControl::*;
    use Source::*;
    ...
    for c in music.ac_recv.try_iter() {
        match c {
            // Set volume provided
            SetVolume(Mic, v) => music.mic_volume = v,
            SetVolume(Line, v) => music.line_volume = v,
            SetVolume(Song, v) => music.song_volume = v,
            ...
            // Pause = true
            Pause => atom!(paused = true),

            // If there is a song selected, pause = false
            Unpause => atom!(paused = music.song.is_none()),
    ...
    if atom!(paused) {
        ...
        for i in 0..out.len() {
            let m = music.mic_volume * mic[i];
            let l = music.line_volume * line[i];
            out[i] = (m + l) / 3.0;
        }
    } else {
        ...
        // Step = number of samples that will be copied into the output buffer. Most calls this will
        // be equal to the length of the output, but if the music data runs out step will be less.
        ...
        for i in 0..step {
            let m = music.mic_volume * mic[i];
            let l = music.line_volume * line[i];
            let s = music.song_volume * song[timestamp + i];
            out[i] = (m + l + s) / 3.0;
        }
